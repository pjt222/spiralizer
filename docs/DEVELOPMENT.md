...
In `R/ui_controls.R`, update choices:
...
In `R/ui_plot.R`, handle the palette:
...
Add to `inst/app/www/css/overlay.css`:
...
# View cache stats (load package first)
devtools::load_all()
get_cache_stats()
...
2. Check style: Consider using `lintr::lint_package()`
3. Update documentation if needed
4. Snapshot dependencies: `renv::snapshot()`

## Deployment

See `inst/scripts/deploy.R` for deployment to shinyapps.io:
